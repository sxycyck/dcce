...
class UpDnNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.bias_lin = torch.nn.Linear(output_dim, 1)    #  transforms multidimensional joint_repr into single demension  
        
        self.smooth_param = torch.nn.Parameter(
              torch.from_numpy(np.full((1,), -1, dtype=np.float32)))   # a trainable parameter smoothing training process.
    ...
    def forward(self, batch):
        ...
        # print('logits',logits)
        out['logits'] = logits
        out['joint_repr'] = self.bias_lin(joint_repr)                               
        out['smooth_param'] = self.smooth_param
