...
class AbstractVQA(Dataset):
    ...
    def __init__(self,
        ...
        self.path_type_to_bias = osp.join(self.subdir_processed,'type_to_bias.pth')
        ...
        self.type_to_bias = torch.load(self.path_type_to_bias)
        ...
        self.collate_fn = bootstrap_tf.Compose([
            ...
            bootstrap_tf.PadTensors(use_keys=[
                'question', 'pooled_feat', 'cls_scores', 'rois', 'cls', 'cls_oh', 'norm_rois'#,'question_'
            ]),
    ...
    def process(self):
        ...
        trainset['annotations'],type_to_bias = self.add_qtype_bias(trainset['annotations'],ans_to_aid)           
        if self.proc_split == 'train' and self.has_valset:
        ...
        torch.save(type_to_bias,self.path_type_to_bias)                             
        if self.proc_split == 'train' and self.has_valset:
    ...
    def add_answers_occurence(self, annotations, ans_to_aid):
        ...
        return annotations
    def add_qtype_bias(self,annotations,ans_to_aid):          
        target = []
        for ans_entry in annotations:
            ...
            for answer in answer_count:
                if answer not in ans_to_aid:
                    continue
                labels.append(ans_to_aid[answer])
        ...
        for ex in annotations:
            ...
            if ex['labels'] is not None:
                for label,score in zip(ex['labels'],ex['scores']):
                    question_type_to_probs[q_type][label]+=score
        question_type_to_prob_array = {}
        answer_voc_size = len(ans_to_aid)
        ...
        return annotations,question_type_to_prob_array                                
